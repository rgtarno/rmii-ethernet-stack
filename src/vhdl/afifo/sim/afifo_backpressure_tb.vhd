...
entity afifo_backpressure_tb is
  generic (runner_cfg : string);
end entity;

architecture tb of afifo_backpressure_tb is
  ...
  signal WRITE_STALL_PROB     : real    := 0.05;
  signal READ_STALL_PROB      : real    := 0.8;
    ...
    while test_suite loop
      ...
      if run("Random read and write") then
        ...
        for i in 0 to num_ops-1 loop
          ...
          wait until rising_edge(clk_a) and write_en = '1' and full = '0';
          data_in_counter <= data_in_counter + 1;
          tmp := std_logic_vector(data_in_counter);
          expect(net, fifo_dout_checker, tmp, now, now + 100 * C_CLK_A_PERIOD);
        end loop;

        -- Stop writes and wait until empty
        start_writes <= '0';
        wait_clk(clk_a, 1);
        wait until rising_edge(clk_b) and empty = '1';
...
begin
  ...
  wait until start_writes = '1' and rising_edge(clk_a);
  ...
  while number_of_writes < num_ops loop
    ...
    write_en <= '1';
    wait until full = '0' and rising_edge(clk_a);
    write_en <= '0';
...
begin
  ...
  wait until start_reads = '1' and rising_edge(clk_b);
  ...
  while start_reads = '1' loop
    ...
    wait until empty = '0' and rising_edge(clk_b);
